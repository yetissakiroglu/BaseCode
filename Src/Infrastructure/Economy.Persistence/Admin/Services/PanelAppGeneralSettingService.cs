...
public class PanelAppGeneralSettingService : IPanelAppGeneralSettingService
{
    ...
    public async Task<ServiceResult<AppGeneralSettingDto>> GetGeneralSettingAsync()
    {
        var entity = await _appGeneralSettingRepository.DataSet.FirstOrDefaultAsync(x=>!x.IsDeleted);
        if (entity == null)
        {
            return ServiceResult<AppGeneralSettingDto>.Empty();
        }

        var dtoModel = new AppGeneralSettingDto()
        {
            Id = entity.Id,
            ...
            MetaTitleSuffix = entity.MetaTitleSuffix,
            DefaultMetaDescription = entity.DefaultMetaDescription,
        };

        return ServiceResult<AppGeneralSettingDto>.Success(dtoModel);
    }
}
