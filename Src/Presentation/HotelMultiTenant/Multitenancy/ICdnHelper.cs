...
namespace HotelMultiTenant.Multitenancy
{
    ...
    {
        private readonly IContentService _cfg;
        public CdnHelper(IContentService cfg) { _cfg = cfg; }

        public string Url(string path, bool withVersion = true)
        {
            var cfg = _cfg.GetTanentAsync().GetAwaiter().GetResult();
            var baseUrl = cfg.Settings?.CdnBaseUrl;
            var enabled = cfg.Settings?.EnableCdn ?? false;
