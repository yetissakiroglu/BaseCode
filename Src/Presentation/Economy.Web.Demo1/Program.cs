...
// (Teşhis) eşleşen endpoint’i consola yaz
app.Use(async (ctx, next) =>
{
    await next();
    Console.WriteLine($"[ROUTE] {ctx.Request.Path} -> {ctx.GetEndpoint()?.DisplayName ?? "(eşleşme yok)"}");
});
...
// (opsiyonel) diğer controller/action rotaları en SONDA kalsın
app.MapControllerRoute(
    name: "default",
